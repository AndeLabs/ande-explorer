...
export default function TokensPage() {
  ...
  const { data, isLoading, error, refetch, isFetching, dataUpdatedAt } = useTokens({ page });
  const [newTokenAddresses, setNewTokenAddresses] = useState<Set<string>>(new Set());
  const prevTokensRef = useRef<string[]>([]);

  // Track new tokens for animation
  useEffect(() => {
    if (data?.items && page === 1) {
      const currentAddresses = data.items.map(token => token.address);
      const prevAddresses = prevTokensRef.current;

      if (prevAddresses.length > 0) {
        const newAddresses = currentAddresses.filter(addr => !prevAddresses.includes(addr));
        if (newAddresses.length > 0) {
          setNewTokenAddresses(new Set(newAddresses));
          setTimeout(() => setNewTokenAddresses(new Set()), 2000);
        }
      }

      prevTokensRef.current = currentAddresses;
    }
  }, [data, page]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ERC-20, ERC-721, and ERC-1155 tokens on {config.chain.name}
          </p>
        </div>
        {page === 1 && (
          <LiveIndicator
